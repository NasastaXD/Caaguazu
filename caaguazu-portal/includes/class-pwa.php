...
class PROMOTUR_PWA {
	...
	/**
	 * Color de marca en hex, para las metas y el manifest (no aceptan var()).
	 * Es la tinta del panel: el acento único del sistema de diseño.
	 */
	public static function brand_hex() {
		return apply_filters( 'promotur_brand_hex', '#101012' );
	}

	/**
	 * Color del cromo del navegador (barra de estado en la PWA). El panel es
	 * claro por defecto, así que la barra acompaña al papel; en modo oscuro,
	 * a la tinta. El <head> imprime las dos con media query.
	 */
	public static function chrome_color( $dark = false ) {
	...
	public function render_manifest() {
		...
		$data = array(
			...
			'background_color' => self::chrome_color(),
			'theme_color'      => self::chrome_color(),
			'lang'             => 'es',
			'icons'            => array(
				array( 'src' => promotur_url( 'icon-192.png' ), 'sizes' => '192x192', 'type' => 'image/png', 'purpose' => 'any' ),
				array( 'src' => promotur_url( 'icon-512.png' ), 'sizes' => '512x512', 'type' => 'image/png', 'purpose' => 'any' ),
				array( 'src' => promotur_url( 'icon-512.png' ), 'sizes' => '512x512', 'type' => 'image/png', 'purpose' => 'maskable' ),
			),
		);
	...
	public function render_sw() {
		...
		// El SW se sirve desde /turismo-panel/sw.js, así que su alcance por
		// defecto ya es /turismo-panel/ — justo lo que queremos ahora que todo
		// el panel vive ahí. No hace falta Service-Worker-Allowed.
		$version  = PROMOTUR_VERSION;
		$offline  = promotur_url( 'offline' );
		$css      = promotur_asset( 'css/caaguazu-portal.css' );
		$js       = promotur_asset( 'js/caaguazu-portal.js' );
		$icon     = promotur_url( 'icon-192.png' );

		// La tipografía entra al precache: sin ella, la pantalla offline se
		// dibuja con otra letra justo cuando el usuario ya está desconfiando.
		$fuentes  = array(
			promotur_asset( 'fonts/inter-400.woff2' ),
			promotur_asset( 'fonts/inter-600.woff2' ),
			promotur_asset( 'fonts/inter-700.woff2' ),
		);

		$precache = wp_json_encode( array_merge( array( $offline, $css, $js, $icon ), $fuentes ) );
...
<head>
	...
	<meta name="theme-color" content="<?php echo esc_attr( self::chrome_color() ); ?>">
	<meta name="theme-color" media="(prefers-color-scheme: dark)" content="<?php echo esc_attr( self::chrome_color( true ) ); ?>">
</head>
