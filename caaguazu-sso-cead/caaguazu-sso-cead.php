...
require_once CEADSSO_DIR . 'includes/class-roles.php';
require_once CEADSSO_DIR . 'includes/class-validacion.php';
...
function ceadsso_admin_notices() {
	...
	if ( ! CEADSSO_Redeem::configured() ) {
		echo '<div class="notice notice-warning"><p>' .
			esc_html__( 'Caaguazú SSO CEAD: faltan CEAD_TUR_SSO_SECRET y/o CEAD_TUR_SSO_URL en wp-config.php. Ver Herramientas → Acceso desde el CEAD.', 'caaguazu-sso-cead' ) .
			'</p></div>';
		return;
	}

	/*
	 * Roles que el CEAD mandó y este lado no supo traducir. Se avisa acá y no
	 * sólo en la pantalla de la integración porque el síntoma lo sufre otra
	 * persona —la que no pudo entrar— y quien lo puede arreglar no tiene
	 * motivo para ir a mirar esa pantalla si nadie le dice.
	 */
	$sin_mapear = CEADSSO_Validacion::roles_sin_mapear();
	if ( $sin_mapear ) {
		printf(
			'<div class="notice notice-warning"><p>%s <a href="%s">%s</a></p></div>',
			esc_html( sprintf(
				/* translators: %s = lista de roles del CEAD */
				__( 'Acceso desde el CEAD: alguien intentó entrar con un rol que no está habilitado (%s).', 'caaguazu-sso-cead' ),
				implode( ', ', $sin_mapear )
			) ),
			esc_url( admin_url( 'tools.php?page=caaguazu-sso-cead' ) ),
			esc_html__( 'Habilitarlo', 'caaguazu-sso-cead' )
		);
	}
}
