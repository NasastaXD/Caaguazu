...
2. Activarlo. Al activar se siembran automáticamente, sin ningún paso manual — el sitio queda navegable de punta a punta sin 404s desde el primer momento:
   - Las páginas requeridas por el theme, en blanco (`sobre-caaguazu`, `ecosistema`, `contacto` con su template de formulario) — ver `inc/core-pages-seeder.php`. Al estar vacías, `page.php` les pinta un hero default + "En construcción" hasta que el admin cargue el contenido real. Esta siembra corre tanto al activar el theme como (con un flag para no repetirse) en `admin_init`, para que sitios ya activos que reciben una actualización in-place también la reciban sin necesidad de reactivar el theme.
...
- **Servicios, Reportá un problema y Directorio de artesanos** existen en el código (CPTs, formularios, templates) pero **no se lanzan todavía** a pedido del departamento — no están en el nav, el footer, los accesos rápidos ni el tabbar. `inc/core-pages-seeder.php` tampoco siembra ya las páginas `servicios`/`reportar`. Para reactivarlos: sumarlos de nuevo a `caaguazu_render_fallback_nav()`/`caaguazu_quick_access_items()` en `inc/helpers.php`, y opcionalmente a `caaguazu_seed_core_pages()`.
...
- **UI de Turismo con cards y botones**: la migración de Fase 1 había dejado los links entre sub-páginas como texto plano dentro de `.entry-content`, dando una sensación de blog. `inc/tourism-content.php` ahora usa `.eco-card`/`.eco-grid` para grillas de atractivos con imagen (ej. "Qué hacer"), `.btn`/`.btn-primary`/`.btn-outline` para los CTA de navegación entre sub-páginas, y un componente nuevo `.info-grid`/`.info-card` para datos cortos (ej. especies madereras) — inspirado en el diseño original del sitio de turismo standalone (`nasastaxd/turismo`), reusando los componentes visuales que ya existían en este theme en vez de introducir CSS nuevo.
