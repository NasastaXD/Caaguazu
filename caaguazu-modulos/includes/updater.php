 ...
 * caaguazu-modulos y caaguazu-turismo (plugins independientes no pueden
 * compartir archivos) — el guard class_exists hace que el segundo en cargar
 * reutilice la clase del primero. Cada plugin la instancia con sus datos
 * desde su bootstrap. Si se toca este archivo, copiar el cambio al gemelo.
 *
 * @package Caaguazu
 */
...
class Caaguazu_Component_Updater {
	...
	// Compartidas entre el theme y ambos plugins: es el mismo release y el
	// mismo manifest — una sola pegada a la API para los tres componentes.
	const RELEASE_CACHE_KEY  = 'caaguazu_github_release';
	const MANIFEST_CACHE_KEY = 'caaguazu_release_manifest';
	const CACHE_TTL          = 12 * HOUR_IN_SECONDS;

	/** @var string p. ej. "caaguazu-modulos/caaguazu-modulos.php" */
	private $basename;
